...
const Users = () => {
  ...
  //   {
  //     id: 2,
  //     avater: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQCd3hMasM5X4rqIDZHTIqLkn_tLml4zHSUdw&usqp=CAU",
  //     name: "Rakib Hossain",
  ...
  const { data, isLoading, isError } = useFetch<IUser[]>(["getUserData", 11], GET_USERS_DATA, { limit: 20, offset: 0 });
  ...
  return (
    <div className="md:mx-10">
      ...
      <AddUserModal title="NOMAN " showModal={showModal} setShowModal={setShowModal} />
      <div className="relative overflow-x-auto shadow-md sm:rounded-sm ">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {data?.payload.map(({ name, email, role, image_url }, index) => (
              <tr key={index + 1} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td className="px-6 py-4 text-left">
                  <div className="flex gap-2  ">
                    <button className="bg-green-500 text-white px-4 py-1 rounded-sm hover:bg-green-600 w-16" onClick={() => setShowModal(true)}>
                      Edit
                    </button>
                    <button className="bg-red-500 text-white px-4 py-1 rounded-sm hover:bg-red-600 w-16">Delete</button>
                  </div>
                </td>
              </tr>
            ))}
